...
touch /etc/nginx/logs/error_nginx.log
rm /etc/nginx/nginx.conf
sudo wget -P /etc/nginx/ https://privacdn.com/lnmpstack/nginx.conf
...
ufw logging low
touch /etc/rc.local
printf '#!/bin/sh\n/usr/sbin/ufw enable' > /etc/rc.local
clear

printf "\033[0;32mInstalling Fail2Ban\033[0m\n"
sleep 1
apt-get -q -y install ufw fail2ban
...
apt-get update
apt-get -q -y install php7.1-fpm php7.1-cli php7.1-curl php7.1-mysql php7.1-sqlite3 php7.1-gd php7.1-xml php7.1-mcrypt php7.1-mbstring php7.1-iconv
...
sleep 1
 apt-get -q -y install zip unzip php7.0-zip
 ...
 cd /var/www/html/
 wget https://privacdn.com/lnmpstack/m4hxc2.txt
 wget -i m4hxc2.txt
 mv index.html index.php
 rm /var/www/html/m4hxc2.txt
 cd
 clear

printf "\033[0;32mFinishing Installation\033[0m\n"
sleep 1
sed -i 's_https://help.ubuntu.com_lnmpstack.com/docs_g' /etc/update-motd.d/10-help-text
sed -i "/\b\(management\|landscape\)\b/d" /etc/update-motd.d/10-help-text
sed -i 's_https://ubuntu.com/advantage_playyground.com/support_g' /etc/update-motd.d/10-help-text
sed -i "/\b\(echo\|ubuntu\)\b/d" /etc/update-motd.d/51-cloudguest
apt-get -q -y update
apt-get -q -y upgrade
...
apt-get -q -y autoclean
apt-get -q -y autoremove
deborphan | xargs apt-get -q -y remove --purge
clear
