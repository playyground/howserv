...
apt-get -q -y install zsh && curl -L http://install.ohmyz.sh | sh && which zsh && chsh -s `which zsh` && sed -i 's/ZSH_THEME="robbyrussell"/ZSH_THEME="agnoster"/g' ~/.zshrc && git clone git://github.com/zsh-users/zsh-autosuggestions $ZSH_CUSTOM/plugins/zsh-autosuggestions && git clone git://github.com/zsh-users/zsh-autosuggestions ~/.zsh/zsh-autosuggestions && sed -i 's/plugins=(git)/plugins=(zsh-autosuggestions)/g' ~/.zshrc && clear
