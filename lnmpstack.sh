...
apt-get -q -y install zsh && apt-get -q -y install git-core && curl -L http://install.ohmyz.sh | sh && which zsh && chsh -s `which zsh` && sed -i 's/ZSH_THEME="robbyrussell"/ZSH_THEME="agnoster"/g' ~/.zshrc && clear
...
rm /var/www/html/index.nginx-debian.html && apt-get -q -y install zip unzip php7.0-zip && curl -sS https://getcomposer.org/installer | php && mv composer.phar /usr/local/bin/composer && cd /var/www/html && rm -f index.nginx-debian.html && composer create-project laravel/laravel . && chown -R www-data:www-data /var/www/html && chmod -R 775 /var/www/html/storage && sed -i 's_root /var/www/html_root /var/www/html/public_g' /etc/nginx/sites-available/default && clear
...
printf "\033[1;33mWould you like to restart your system? (y/n)\033[0m\n"
while true; do
    read yn
    case $yn in
        [Yy]* ) rm -- "$0"; reboot;;
