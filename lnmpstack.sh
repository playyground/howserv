...
SSH="$(shuf -i 1024-65535 -n 1)" && sed -i 's/Port 22/Port $SSH/g' /etc/ssh/sshd_config && /etc/init.d/ssh reload && sleep 2 && clear

printf "\033[0;32mInstalling Nginx-Extras Module\033[0m\n"; sleep 2
...
yes | ufw enable && ufw allow 443/tcp && ufw limit 1500/tcp && ufw allow 80/tcp && ufw default deny incoming && ufw default allow outgoing && ufw logging low && touch /etc/rc.local && printf '#!/bin/sh\n/usr/sbin/ufw enable' > /etc/rc.local && clear && ufw status verbose && sleep 3 && clear
...
printf "\033[0;32mInstalling Laravel\033[0m\n"; sleep 2
rm /var/www/html/index.nginx-debian.html && apt-get install zip unzip php7.0-zip -y && cd ~ && curl -sS https://getcomposer.org/installer | php && mv composer.phar /usr/local/bin/composer && cd /var/www/html && rm -f index.nginx-debian.html && composer create-project laravel/laravel . && chown -R www-data:www-data /var/www/html && chmod -R 775 /var/www/html/storage && sed -i 's_root /var/www/html_root /var/www/html/public_g' /etc/nginx/sites-available/default
