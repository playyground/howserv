...
apt-get install nginx-extras -y && sleep 2 && mkdir /etc/nginx/logs/ && touch /etc/nginx/logs/error_nginx.log && sudo wget -P /etc/nginx/ https://privacdn.com/howserv/nginx.conf && truncate -s 0 /etc/nginx/sites-available/default && printf 'server { listen 80; root /var/www/html; server_name _; index index.php index.html index.htm; location / { try_files $uri $uri/ /index.php$is_args$args; } location ~ \.php$ { incude snippets/fastcgi-php.conf; fastcgi_pass unix:/run/php/php7.1-fpm.sock; } }' > /etc/nginx/sites-available/default
